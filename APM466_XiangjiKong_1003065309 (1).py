...
for i in range(0,10):
    for j in range(0,11):
        ...
        if j == 0:
            spot_rates[i][j] = -np.log(dirty_prices[i][j]/(100*float(df.iloc[j,0])/2 + 100)) /(maturities[i][j])
        else:
        # Considering the cases of the 6th and the 8th bond where we have irregular months of bonds, which divide the time span of bonds into two parts.
            if j!= 5 and j != 7:
                payments = np.asarray([float(bonds.iloc[j,0])/2 *100] * j + [100*float(bonds.iloc[j,0])/2 + 100])
            else:
                payments = np.asarray([float(bonds.iloc[j,0])/2 *100] * (j-1) + [100*float(bonds.iloc[j,0])/2 + 100])
        ...
        # Considering the spot rates of the rest of bonds (i.e. the 6th,7th,8th,9th,10th,11th bonds)
                ...
                rate = np.asarray([(spot_rates[i][n+1])/2 + spot_rates[i][n]  for n in range(0,4)])
            elif j == 6:
                time = np.asarray([maturities[i][n] for n in range(0,5)] + [maturities[i][5] + 1/4])
                rate = np.asarray([2* spot_rates[i][5] - spot_rates[i][4]] +[spot_rates[i][n] for n in range(0,5)]  )    
            elif j == 7:
                ...
                rate = np.asarray([spot_rates[i][8]] +[spot_rates[i][9]]+ [spot_rates[i][n] for n in range(0,5)] + [2* spot_rates[i][5] - spot_rates[i][4]] + [spot_rates[i][6]] +[2* spot_rates[i][7] - spot_rates[i][6]])
